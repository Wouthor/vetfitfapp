    ...
    <header className="bg-ink sticky top-0 z-50">
      <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
        <Link href={role === 'instructor' ? '/instructor' : '/athlete'} className="flex items-baseline space-x-3">
          ...
          <span className="font-label font-bold text-[11px] uppercase tracking-widest text-rose">
        ...
        <div className="flex items-center space-x-4">
          {email && (
            <span className="text-sm text-rose hidden sm:block truncate max-w-32">
              {email}
            </span>
          )}
          <button
            onClick={handleLogout}
            className="font-label font-bold text-xs uppercase tracking-widest text-rose hover:text-paper transition-colors"
          >
            Uitloggen
          </button>
        </div>
